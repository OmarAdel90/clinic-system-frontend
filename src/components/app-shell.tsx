...
export function AppShell({ user, children }: AppShellProps) {
  ...
  return (
    <div className="min-h-screen bg-[var(--surface)] text-slate-900">
      ...
      >
        <aside className="mb-4 rounded-2xl border border-[var(--line)] bg-[var(--panel)] p-4 shadow-[var(--shadow-soft)] lg:mb-0 lg:w-[300px] lg:flex-none">
          <div className="flex items-start justify-between gap-4 border-b border-[var(--line)] pb-4">
            <div className={isRTL ? "text-right" : ""}>
              <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-slate-500">
                {t("Clinic System")}
              </p>
              <h1 className="mt-2 text-xl font-semibold text-slate-950">{t("Operations CRM")}</h1>
          ...
          <nav className="mt-4 space-y-4">
            {visibleGroups.map((group) => (
              <div key={group.label} className="rounded-xl border border-[var(--line)] bg-[var(--surface)]/70 p-2.5">
                <div className={`px-2 pb-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 ${isRTL ? "text-right" : ""}`}>
                  {t(group.label)}
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div className={`${active ? "text-white" : "text-slate-800"} text-sm font-medium ${isNestedVisitItem ? "text-[13px]" : ""}`}>{t(item.label)}</div>
                        </div>
                        <div
                          className={`mt-1 leading-5 ${active ? "text-slate-200" : "text-slate-500"} ${isNestedVisitItem ? "pl-5 text-[11px]" : "text-[11px]"} ${isRTL ? "text-right" : ""}`}
                        >
                          {t(item.description)}
                        </div>
                      </Link>
